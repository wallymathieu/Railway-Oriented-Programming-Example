...
let succeed x : RopResult<_,_>=
    Success (x,[])

/// create a Success with a message
let succeedWithMsg x msg : RopResult<_,_>=
    Success (x,[msg])

/// create a Failure with a message
let fail msg : RopResult<_,_>=
    Failure [msg]
...
let mergeMessages msgs result : RopResult<_,_>=
...
let bindR f result : RopResult<_,_>=
    let fSuccess (x,msgs) = 
...
let applyR f result : RopResult<_,_>=
    match f,result with
...
let successTee f result : RopResult<_,_>= 
    let fSuccess (x,msgs) = 
        f (x,msgs)
        Success (x,msgs) 
...
/// and pass thru the result
let failureTee f result : RopResult<_,_>= 
...
let mapMessagesR f result : RopResult<_,_>= 
    match result with 
    | Success (x,msgs) -> 
